...
const LineItemBasePriceMaybe = props => {
  const { lineItems, code, intl, breakdownData } = props;
  ...
  const start = moment(breakdownData.startDate);
  const end = moment(breakdownData.endDate);
  ...
  const quantity = unitPurchase?.units
    ...
    : unitPurchase?.quantity
      ...
      : null;
  const updatedQuantity = quantity && differenceInDays === 3 ? 3 : quantity;
  ...
  const message = unitPurchase?.seats ? (
    <FormattedMessage
      ...
      values={{ unitPrice, quantity: updatedQuantity, seats: unitPurchase.seats }}
    />
  ) : (
    <FormattedMessage id={translationKey} values={{ unitPrice, quantity: updatedQuantity }} />
  );
