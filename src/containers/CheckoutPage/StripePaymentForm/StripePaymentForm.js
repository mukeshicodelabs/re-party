...
const LocationOrShippingDetails = props => {
  ...
  const { rentPickupLocation: { selectedPlace: { address: rentPickupAddress }={} } = {}, rentShippingLocation: { selectedPlace: { address: rentShippingAddress }={} } = {}, pickupAddressSell, deliveryAddressSell } = listing?.attributes?.publicData || {};
  const { sellPickupAddress, sellPickupCity, sellPickupState, sellPickupZipCode } = pickupAddressSell || {};
  const { sellShippingAddress, sellShippingCity, sellShippingState, sellShippingZipCode } = deliveryAddressSell || {}; 
  ...
  const locationDetails = rentPickupAddress && deliveryMethod == 'pickup' &&
    productType == 'rent' ? rentPickupAddress : rentShippingAddress &&
      productType == 'rent' && deliveryMethod == 'delivery' ? rentShippingAddress :
    productType == 'sell' && (deliveryMethod == 'pickup'|| !deliveryfee) ? `${sellPickupAddress},${sellPickupCity},${sellPickupState},${sellPickupZipCode}` :
      productType == 'sell' && (deliveryMethod == 'delivery'||deliveryfee) ? `${sellShippingAddress}, ${sellShippingCity}, ${sellShippingState}, ${sellShippingZipCode}`
