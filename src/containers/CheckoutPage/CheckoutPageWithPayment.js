...
export const getOrderParams = (pageData, shippingDetails, optionalPaymentParams, config) => {
  ...
  const productType = pageData?.orderData?.productType || null;
  ...
  const protectedDataMaybe = {
    protectedData: {
      ...
      selectedShippingOption,
       ...(productType === 'sell' && deliveryMethod === 'delivery'
    ? {
        userShippingAddressShippo: allAddressFieldsExist
          ? `${apt ? apt + ' ' : ''}${deliveryAddress}, ${city}, ${state}, ${zip}`
          : userDeliveryLocation,
      }
    : {}),
